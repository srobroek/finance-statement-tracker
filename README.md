...
Six Codex automations are active: one hourly live transaction/evidence ingest at minute 5 and five source-specific monthly statement jobs for RAKBANK World, Standard Chartered Platinum X, Emirates Islamic Amazon, ADCB, and Wio. Cashback card cycles are tentatively month-end, with the monthly jobs staggered on the following first day. The former daily aggregate gate is paused. RAKBANK and Standard Chartered jobs deliberately leave their periods open while their statement contracts remain placeholders; ADCB, Emirates Islamic, and Wio have verified statement sources. Every job is idempotent and leaves cursors, Actual, or close state unchanged when a required connector, source contract, parser, account mapping, or evidence gate is unavailable. The hourly Sol job scans the complete durable-cursor gap plus overlap and submits exact Outlook message objects through `push-outlook-messages.ps1`; the continuous companion performs deterministic parsing, static rules, deduplication, persistence, and dashboard refresh in one acknowledged call. Sol handles only unresolved classification and related-email evidence after that deterministic pass. Failed payloads remain under `runtime` for retry.
