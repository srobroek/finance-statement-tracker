...
must be retired only by exact reviewed signatures during the final rule audit.
The transaction note grammar and final cleanup gate are documented in
`docs/actual-note-contract.md`.
...
| 20:40 on day 3 | Wio statement | Map card suffix `4113` and account suffix `5009` to the single `Wio Credit Card · 4113 / 5009` Actual account, then stage and commit only when every row maps cleanly. Wio's generic 2% reward is intentionally excluded from live routing. |
...
After statement normalization, every card-specific job performs a selective evidence pass over the closed period using `config/evidence-search-policy.json`. A transaction qualifies through an explicit evidence request, an always-search category, or the configured AED-equivalent value threshold while outside the never-search categories. Store examples include warranty-bearing electronics and appliances, furniture, property and utility invoices, contractor records, travel bookings, insurance or professional documents, and high-value online orders with useful purchase terms. Do-not-store examples include supermarket or convenience baskets, cafe/restaurant/delivery charges, petrol/parking/tolls/transport, cash or transfer records, card payments and fees, and low-value retail without warranty, contract, tax, or ownership value.

Because a statement may identify only the vendor, the job searches vendor aliases together with exact amount/currency and the posting date inside the configured buffer, adding card/account identity or an order/reference when available. It requires at least two strong facts and never accepts vendor similarity alone. Foreign-currency candidates can use the original currency and a displayed statement exchange rate; related rows may be matched as a group against one booking or receipt total. A confirmed email is then used to extract the product or service description, order reference, model, serial number, warranty dates, and return deadline when present. Original attachments are preferred; a confirmed eligible email without one is rendered to a deterministic redacted text snapshot. `outlook-evidence-snapshot` creates that snapshot and `purchase-evidence-archive` links it to one or more exact transaction identities. The result is hash-deduplicated under `Finance Evidence/YYYY/MM/vendor-slug/`, recorded in `Finance Evidence/catalogue.json`, and linked through the transaction evidence metadata. Ambiguous candidates remain unmatched and review-required.

The Actual bootstrap config may list obsolete zero-balance accounts in `retired_accounts`. Applying bootstrap closes those accounts only after verifying an exact name match and a zero balance; a non-zero retired account fails closed. This keeps declarative account cleanup reproducible without risking transaction-bearing accounts.
