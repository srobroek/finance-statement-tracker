...
Operationally, the split is hidden: Actual, Cashback Control, and the ingestion worker are independent containers; monthly statement jobs and one active morning durable-cursor transaction job call stable commands, and no Windows background process is required.
