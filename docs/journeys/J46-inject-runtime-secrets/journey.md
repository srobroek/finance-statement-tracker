...
## Preconditions
- The target runtime and exact workflow/run are identified.
- MFA/OTP remains a user boundary; the operator has approved vault scope and a protected pre-state receipt.
- The exclusive write lane, rollback/compensation path, and redacted receipt location are available.

## Surfaces and interfaces
n8n credential/runtime configuration, finance host environment, Codex runner, and protected audit receipt. This is RW-O: secret injection is a consequential write; all inspection-only checks must prove no mutation.
