...
Deploy `compose.yaml` as the Dockge-compatible stack `finance-actual`.
This is the production topology:

- `finance-actual`: the digest-pinned Actual 26.9.0 ledger service,
...
Actual data is bind-mounted at `./data`. Cashback operational state is owned by
the independent `/opt/stacks/finance-cashback` project.
