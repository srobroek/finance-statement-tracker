...
def load_adapter():
    spec = importlib.util.spec_from_file_location(
        "finance_n8n_application_adapter", INTERFACE / "adapter.py"
    )
...
class N8nApplicationInterfaceTests(unittest.TestCase):
    ...
    def test_finance_adapter_emits_the_strict_generic_manifest(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(
                manifest["fixtures"],
                {"directory": "fixtures", "manifest": "fixtures/fixture-manifest.json"},
            )
            self.assertEqual(
            ...
            self.assertEqual(
                ...
                {
                    row["placeholder"]
                    for row in load_json(N8N / "credential-bindings.json")["bindings"]
                },
            )
            bindings = load_json(N8N / "credential-bindings.json")["bindings"]
            self.assertEqual(len(bindings), 8)
            self.assertEqual(sum(len(row["nodes"]) for row in bindings), 46)
            self.assertEqual(
                manifest["route"],
                {
    ...
    def test_staged_credential_contract_is_exact_and_stable(self) -> None:
        with (
            ...
            self.assertEqual(
                [
                    path.read_bytes()
                    for path in sorted((first_root / "workflows").glob("*.json"))
                ],
                [
                    path.read_bytes()
                    for path in sorted((second_root / "workflows").glob("*.json"))
                ],
            )
    ...
    def test_adapter_rejects_stale_credential_contract_declaration(self) -> None:
        ...
        with (
            ...
            patch.object(self.adapter, "_sha256", side_effect=stale_contract),
        ):
            with self.assertRaisesRegex(
                ValueError, "credential binding declaration is stale"
            ):
                self.adapter.stage_application(
                    ROOT, Path(temporary) / "application", "a" * 40
                )
    ...
    def test_adapter_rejects_secret_bearing_credential_placeholder_extra(self) -> None:
        ...
        with (
            tempfile.TemporaryDirectory() as temporary,
            patch.object(self.adapter, "_load", side_effect=load),
        ):
            with self.assertRaisesRegex(
                ValueError, "credential placeholder is invalid"
            ):
                self.adapter.stage_application(
                    ROOT, Path(temporary) / "application", "a" * 40
                )
    ...
    def test_adapter_rejects_duplicate_credential_placeholder_binding(self) -> None:
        ...
        with (
            ...
            with self.assertRaisesRegex(
                ValueError, "credential bindings are duplicated"
            ):
                self.adapter.stage_application(
                    ROOT, Path(temporary) / "application", "a" * 40
                )
    ...
    def test_workflow_count_and_folder_contract_are_exact(self) -> None:
        ...
        for row in registry_rows:
            ...
            self.assertEqual(
                by_code[code]["id"], workflow["meta"]["workflowFolder"]["id"]
            )
    ...
    def test_finance_application_manifest_binds_the_generic_input_corpus(self) -> None:
        ...
        self.assertEqual(
            ...
            "d160aa0c29a1564c9e54ab4eec1ab4f28be89dd0",
        )
        self.assertEqual(manifest["inactive_corpus"]["file_count"], 19)
        self.assertEqual(
            manifest["workflow_manifest"]["path"],
            "integrations/n8n/pipeline-registry.json",
        )
        self.assertEqual(
            manifest["fixture_manifest"]["path"],
            "integrations/n8n/disposable/fixture-manifest.json",
        )
        self.assertEqual(
            manifest["workflow_manifest"]["sha256"],
            sha256(N8N / "pipeline-registry.json"),
        )
        self.assertEqual(
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(
                len(staged["workflows"]["files"]),
                manifest["inactive_corpus"]["file_count"],
            )
            self.assertEqual(
                load_json(
                    Path(temporary) / "application" / "fixtures/fixture-manifest.json"
                ),
    ...
    def test_data_table_schema_and_adapter_projection_are_exact(self) -> None:
        errors = sorted(
            Draft202012Validator(
                self.table_schema, format_checker=FormatChecker()
            ).iter_errors(self.tables),
            key=str,
        )
        ...
        self.assertEqual(
            {row["name"] for row in self.tables["tables"]},
            {
                ...
                if row["name"].startswith("finance_")
            },
        )
        with tempfile.TemporaryDirectory() as temporary:
    ...
    def test_onedrive_root_and_folder_fixture_contract_stays_finance_owned(
        self,
    ) -> None:
        setup_root = N8N / "setup-workflows"
        workflow = load_json(
            setup_root / "22-onedrive-finance-evidence-root-setup.json"
        )
        ...
        self.assertEqual(
            setup_manifest["publication_policy"],
            "PER_WORKFLOW_REVIEWED_DEPENDENCY_ONLY",
        )
        setup_rows = {row["code"]: row for row in setup_manifest["workflows"]}
        self.assertEqual(
            set(setup_rows),
            {
                "ONEDRIVE_FINANCE_EVIDENCE_ROOT_SETUP",
                "MICROSOFT_OAUTH_REFRESH_PROOF",
                "ADCB_REVIEWED_MAINTENANCE",
            },
        )
        ...
        for setup_code, row in setup_rows.items():
            ...
            if setup_code != "ADCB_REVIEWED_MAINTENANCE":
                ...
                self.assertIs(
                    exported["meta"]["reviewedIntegratedInvocationRequired"], True
                )
                triggers = [
                    node["type"]
                    for node in exported["nodes"]
                    if node["type"].endswith("Trigger")
                    or node["type"].endswith("webhook")
                ]
                self.assertEqual(triggers, ["n8n-nodes-base.executeWorkflowTrigger"])
        assignments = {
            row["name"]: row["value"]
            for row in next(
                node for node in workflow["nodes"] if node["name"] == "Setup Parameters"
        ...
        self.assertFalse(
            any(node_type.startswith("n8n-nodes-finance.") for node_type in node_types)
        )
        code = "\n".join(
            node["parameters"].get("jsCode", "") for node in workflow["nodes"]
        )
        for marker in (
    ...
    def test_disposable_fixture_manifest_binds_exact_integration_inputs(self) -> None:
        ...
        self.assertEqual(len(fixture["workflows"]), 20)
        for filename, digest in fixture["source_workflow_sha256"].items():
            self.assertEqual(digest, sha256(WORKFLOWS / filename))
        self.assertEqual(
            fixture["scenario_contract"]["sweep_zero"]["expected"],
            {"scanned_count": 0, "heartbeat": True},
        )
        self.assertEqual(
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(
                ...
                sha256(
                    N8N / "setup-workflows/22-onedrive-finance-evidence-root-setup.json"
                ),
            )
            self.assertEqual(
    ...
    def test_adapter_rejects_unpinned_source_commits(self) -> None:
        with (
            ...
            self.adapter.stage_application(
                ROOT, Path(temporary) / "application", "finance"
            )
