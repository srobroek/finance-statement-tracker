...
`n8n-nodes-prodex@0.5.1` provides device login and SDK `outputSchema` for ChatGPT/Codex subscription execution. Exact npm integrity values live in `integrations/n8n/community-node-lock.json`. The main AI workflow never exposes community-node parameters: workflow 21 owns the fixed model, prompt template, sandbox, thread mode, timeout, and schema. Callers cannot select provider, model, prompt, command, path, credential, working directory, sandbox, or URL.

The package review found activation gates. ProDex exposes general coding-agent controls in its UI, so this project uses only its fixed read-only/new-thread/schema-bound configuration. Keep the package outside production until exact-image registration, subscription authentication, and schema-bound execution pass.
