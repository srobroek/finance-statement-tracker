...
readonly remediation_mode="${FINANCE_WF23_STRANDED_REMEDIATION_MODE:-}"
case "${remediation_mode}" in
  REHEARSAL)
    [[ "${FINANCE_WF23_STRANDED_REMEDIATION_ACK:-}" == "REHEARSE_EXACT_WF23_TRANSACTION_AND_ROLL_BACK" ]] || {
    ...
    ;;
  COMMIT)
    [[ "${FINANCE_WF23_STRANDED_REMEDIATION_ACK:-}" == "REMOVE_EXACT_WF23_AND_ONE_SOFT_DELETED_ORPHAN" ]] || {
    ...
    ;;
  *)
    echo "Set FINANCE_WF23_STRANDED_REMEDIATION_MODE to REHEARSAL or COMMIT" >&2
    exit 1
    ;;
esac
...
readonly sql_file="${runner_dir}/remediate-stranded-wf23.sql"
...
for helper in canonicalize-wf23-source.py remediate-stranded-wf23.sql prove-wf23-orphan-inactive.sh validate-wf23-rehearsal-receipt.py n8n-cli-finance-data-table-digest.cjs parse_n8n_redacted_wrapper_output.py; do
...
n8n_paused="false"
restore_n8n_if_paused() {
  if [[ "${n8n_paused:-false}" == "true" ]]; then
    docker unpause "${n8n_container}" >/dev/null 2>&1 || true
  fi
}
trap restore_n8n_if_paused EXIT
...
readonly inactivity_receipt='WF23 orphan inactivity verified:{"schema_version":1,"status":"VERIFIED","scope":"WF23_PROCESS_LOCAL_ACTIVE_EXECUTIONS_ABSENCE","stdin_node_processes":0,"n8n_initialized":false,"database_initialized":false,"workflow_loaded":false,"provider_calls":false,"secret_values_recorded":false}'
observed_inactivity="$(docker exec -i -e FINANCE_WF23_ORPHAN_PROCESS_PROOF_ACK=PROVE_WF23_EXECUTOR_ABSENT_READ_ONLY "${n8n_container}" sh -s < "${runner_dir}/prove-wf23-orphan-inactive.sh" 2>/dev/null)"
[[ "${observed_inactivity}" == "${inactivity_receipt}" ]] || { echo "WF23 process-local ActiveExecutions absence not proven" >&2; exit 1; }
...
wf23_execution_data_count() { psql_scalar "select count(*) from execution_data d join execution_entity e on e.id=d.\"executionId\" where e.id=${execution_id} and e.\"workflowId\"='${workflow_id}';"; }
wf23_orphan_signature_count() { psql_scalar "select count(*) from execution_entity e where e.id=${execution_id} and e.\"workflowId\"='${workflow_id}' and e.status='running' and e.finished=false and e.mode='cli' and e.\"waitTill\" is null and e.\"stoppedAt\" is null and e.\"deletedAt\" is not null and e.\"storedAt\"='db';"; }
...
[[ "$(project_state)" == "22|0|0" && "$(mapped_count)" == "22" && "$(tag_edge_count)" == "66" && "$(bad_project_tag_set_count)" == "0" ]] || { echo "Expected exact stranded 22/0/0, 22-folder, 66-tag boundary" >&2; exit 1; }
[[ "$(setup_id_count)" == "1" && "$(wf23_workflow_count)" == "1" && "$(wf23_execution_count)" == "1" && "$(wf23_execution_data_count)" == "1" && "$(wf23_orphan_signature_count)" == "1" && "$(wf23_history_count)" == "1" ]] || { echo "Expected exactly one stranded WF23 workflow, orphan execution, execution-data, and history row" >&2; exit 1; }
...
if [[ "${remediation_mode}" == "COMMIT" ]]; then
  rehearsal_receipt="${FINANCE_WF23_REHEARSAL_RECEIPT:-}"
  ...
  receipt_validation="$(python3 "${runner_dir}/validate-wf23-rehearsal-receipt.py" \
  ...
  [[ "${receipt_validation}" == "WF23_REHEARSAL_RECEIPT_VERIFIED" ]] || { echo "Verified recent WF23 rehearsal receipt required" >&2; exit 1; }
fi

# Quiesce the sole retained n8n writer for both the rehearsal and the eventual
# commit. Revalidate the exact live state after the pause; the SQL then repeats
# its exact checks while holding its advisory/table locks. The EXIT trap makes
# every ordinary failure path attempt to resume n8n.
docker pause "${n8n_container}" >/dev/null
n8n_paused="true"
[[ "$(docker inspect -f '{{.State.Running}}|{{.State.Paused}}' "${n8n_container}")" == "true|true" ]] || { echo "n8n quiescent window not established" >&2; exit 1; }
...
[[ "$(project_state)" == "22|0|0" && "$(mapped_count)" == "22" && "$(tag_edge_count)" == "66" && "$(bad_project_tag_set_count)" == "0" ]] || { echo "Stranded boundary changed before locked transaction" >&2; exit 1; }
[[ "$(setup_id_count)" == "1" && "$(wf23_workflow_count)" == "1" && "$(wf23_execution_count)" == "1" && "$(wf23_execution_data_count)" == "1" && "$(wf23_orphan_signature_count)" == "1" && "$(wf23_history_count)" == "1" ]] || { echo "Exact orphan rows changed before locked transaction" >&2; exit 1; }
...
docker unpause "${n8n_container}" >/dev/null
n8n_paused="false"
...
if [[ "${remediation_mode}" == "REHEARSAL" ]]; then
  [[ "$(project_state)" == "22|0|0" && "$(mapped_count)" == "22" && "$(tag_edge_count)" == "66" && "$(bad_project_tag_set_count)" == "0" ]] || { echo "Rollback rehearsal did not restore the stranded workflow boundary" >&2; exit 1; }
  [[ "$(setup_id_count)" == "1" && "$(wf23_workflow_count)" == "1" && "$(wf23_execution_count)" == "1" && "$(wf23_execution_data_count)" == "1" && "$(wf23_orphan_signature_count)" == "1" && "$(wf23_history_count)" == "1" ]] || { echo "Rollback rehearsal did not restore the exact orphan rows" >&2; exit 1; }
  ...
  receipt="${receipt_root}/wf23-postgresql-rollback-rehearsal-${expected_finance_commit:0:12}-$(date -u +%Y%m%dT%H%M%SZ).json"
...
path.chmod(0o600)
PY
  unset observed_inactivity workflow_digest_before corpus_digest_before credential_digest_before credential_digest_sha256_before data_table_digest_before data_table_digest_after sql_sha256 expected_workflow_b64 expected_history_b64 expected_version_id
  echo "Exact WF23 PostgreSQL rollback rehearsal verified; redacted receipt: ${receipt}"
  exit 0
fi
...
[[ "$(setup_id_count)" == "0" && "$(wf23_execution_count)" == "0" && "$(wf23_history_count)" == "0" ]] || { echo "WF23 cleanup readback mismatch" >&2; exit 1; }
...
receipt="${receipt_root}/wf23-stranded-remediation-${expected_finance_commit:0:12}-$(date -u +%Y%m%dT%H%M%SZ).json"
...
payload = {
    "schema_version": 1,
    "status": "VERIFIED",
    "scope": "WF23_EXACT_STRANDED_STATE_REMEDIATION",
    "observed_execution_state": "ORPHANED_SOFT_DELETED_EXECUTION",
    "recorded_at_utc": datetime.datetime.now(datetime.timezone.utc).isoformat(),
    "commits": {"finance": finance_commit, "orchestrator": orchestrator_commit},
    "workflow_source_sha256": source_sha256,
    "workflow_removed": True,
    "orphan_execution_rows_removed": 1,
    "history_rows_removed": 1,
    "process_local_active_executions_owner_absent": True,
...
path.chmod(0o600)
PY
unset observed_inactivity workflow_digest_before corpus_digest_before credential_digest_before credential_digest_sha256_before data_table_digest_before data_table_digest_after sql_sha256 expected_workflow_b64 expected_history_b64 expected_version_id rehearsal_receipt receipt_root_real receipt_validation
echo "Exact stranded WF23 remediation verified; redacted receipt: ${receipt}"
