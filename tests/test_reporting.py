...
class ReportingTests(TestCase):
    def test_shared_owner_report_reuses_canonical_transactions(self) -> None:
        ...
        self.assertEqual(
            [(row.key, row.spend_aed) for row in report],
            [("Shopping", Decimal("100")), ("Dining", Decimal("25"))],
        )
    ...
    def test_unresolved_rows_are_excluded_but_resolved_general_remains(self) -> None:
        rows = [
            Transaction(
                "resolved",
                datetime(2026, 8, 1),
        ...
        result = breakdown(rows, dimension="category")

        self.assertEqual(
            [(item.key, item.spend_aed) for item in result],
            [("GENERAL", Decimal("100"))],
        )
    ...
    def test_tag_filters_support_any_all_and_none_case_insensitively(self) -> None:
        ...
        self.assertTrue(
            ReportFilter(tags_any=frozenset({"business", "shared"})).matches(row)
        )
        self.assertTrue(
            ReportFilter(tags_all=frozenset({"shared", "rental"})).matches(row)
        )
