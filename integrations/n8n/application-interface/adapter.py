...
SOURCE_FILES = {
    ...
    "application_contract_bundle": Path(
        "integrations/n8n/generated/application-contract-bundle.json"
    ),
    "application_contract_schema": Path(
        "integrations/n8n/generated/application-contract-bundle.schema.json"
    ),
    "source_contract_bindings": Path("integrations/n8n/source-contract-bindings.json"),
}
...
def _contract_manifest(finance_root: Path) -> dict[str, object]:
    """Project the generated contract chain without rewriting producer output."""

    bundle_path = finance_root / "generated" / "application-contract-bundle.json"
    schema_path = finance_root / "generated" / "application-contract-bundle.schema.json"
    source_bindings_path = finance_root / "source-contract-bindings.json"
    bundle = _load(bundle_path)
    _load(schema_path)
    source_bindings = _load(source_bindings_path)
    if bundle.get("contract_status") != "SPEC_ONLY":
        raise ValueError("finance application contract must remain SPEC_ONLY")
    if (
        bundle.get("schema_path")
        != "integrations/n8n/generated/application-contract-bundle.schema.json"
    ):
        raise ValueError("finance application contract schema path changed")
    if bundle.get("schema_sha256") != _sha256(schema_path):
        raise ValueError("finance application contract schema is stale")
    source_contract = source_bindings.get("source")
    if (
        not isinstance(source_contract, dict)
        or source_contract.get("path")
        != "integrations/n8n/generated/application-contract-bundle.json"
        or source_contract.get("sha256") != _sha256(bundle_path)
        or source_bindings.get("contract_status")
        not in {"READY", "ACTIVATION_REQUIRED"}
    ):
        raise ValueError("finance source contract binding is stale")
    return {
        "application_bundle": {
        ...
        "source_bindings": {
            ...
            "sha256": _sha256(source_bindings_path),
        },
        "contract_status": "SPEC_ONLY",
        "source_contract_status": source_bindings.get("contract_status"),
    }


def _validate_inactive_workflow(workflow: dict[str, object], filename: str) -> None:
    if workflow.get("active") is not False:
        raise ValueError(f"finance workflow must remain inactive: {filename}")
    metadata = workflow.get("meta")
    if not isinstance(metadata, dict) or metadata.get("migrationStatus") != "SPEC_ONLY":
        raise ValueError(f"finance workflow must remain SPEC_ONLY: {filename}")
...
def stage_application(source_root: Path, destination: Path, source_commit: str) -> Path:
    ...
    for workflow in workflows:
        _validate_inactive_workflow(_load(workflow), workflow.name)
    ...
    contracts = _contract_manifest(finance_root)
    ...
    for key, relative_path in SOURCE_FILES.items():
        _copy_regular(
            ...
            / {
                ...
                "application_contract_bundle": "contracts/application-contract-bundle.json",
                "application_contract_schema": "contracts/application-contract-bundle.schema.json",
                "source_contract_bindings": "contracts/source-contract-bindings.json",
            }[key],
        )
