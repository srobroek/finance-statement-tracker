...
ACQUIRE = r"""
const b = $('Validate Reviewed Bindings').first().json;
return [{json:{operation:'ACQUIRE',lease_class:'MAINTENANCE',resource_key:'actual:'+b.actual_file_id,lease_owner:'n8n:maintenance:'+String($execution.id),ttl_seconds:600}}];
"""
ASSERT = r"""
const l = $('Acquire Global Actual Writer Fence').first().json;
const b = $('Validate Reviewed Bindings').first().json;
const expiresAt = Date.parse(l.expires_at);
if (!Number.isFinite(expiresAt) || l.lease_class !== 'MAINTENANCE' || l.resource_key !== 'actual:'+b.actual_file_id || !l.lease_id || !Number.isSafeInteger(l.fencing_token) || l.fencing_token <= 0 || expiresAt <= Date.now()+60000) throw new Error('MAINTENANCE_WRITER_FENCE_INVALID');
return [{json:{operation:'ASSERT',lease_class:'MAINTENANCE',resource_key:l.resource_key,lease_id:l.lease_id,fencing_token:l.fencing_token}}];
"""
ENVELOPE = r"""
const l = $('Acquire Global Actual Writer Fence').first().json;
...
return [{json:{operation:'RELEASE',lease_class:'MAINTENANCE',resource_key:l.resource_key,lease_id:l.lease_id,fencing_token:l.fencing_token}}];
"""
RETURN = r"""
...
def build():
    ...
    def add(name, kind, parameters, version=1, credentials=None):
        ...
        node = {
            "id": str(25000 + i),
            "name": name,
            "type": "n8n-nodes-base." + kind if "." not in kind else kind,
            "typeVersion": version,
            "position": [i * 260, 0],
    ...
    def code(name, body):
        add(
            ...
            "code",
            {
                "jsCode": "// Purpose: "
                + name
                + ". Keep this deterministic and fail closed.\n"
                + body.strip()
                + "\n"
            },
            2,
    ...
    assignments = [
        {"id": k, "name": k, "type": "string", "value": v} for k, v in bindings.items()
    ]
    assignments += [
        {"id": k, "name": k, "type": "number", "value": -1}
        for k in ["expected_after_count", "expected_after_balance_minor"]
    ]
    add(
    ...
    add(
        ...
        "convertToFile",
        {
            ...
            "mode": "each",
            "binaryPropertyName": "data",
            "options": {"fileName": "maintenance-receipt.json"},
        },
        1.1,
    )
    add(
...
def main():
    ...
    if args.write:
        ...
        raise SystemExit("reviewed maintenance workflow drift")
