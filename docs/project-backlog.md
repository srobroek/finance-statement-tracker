...
Progress recorded: **2026-08-20** at `a07c5ab`.
Evidence commits: `a07c5ab`, `4a30c7f`, `8149f42`, `b07c410`, `00491aa`, `c8d4f7c`, `3a6acc6`, `da6b0c1`, `b845ccf`, `fa8fd58`
...
| 3 | `N8N-002` | `n8n_workflows` | P1 | `PARTIAL` | After exact WF23 cleanup, implement the Finance hierarchy; use Tidy Workflow only for optional coordinate cleanup. |
...
| 17 | `N8N-012` | `n8n_workflows` | P0 | `PARTIAL` | Execute the exact cleanup gate and retain the later content-addressed cleanup receipt before WF22 or N8N-010. |
| 18 | `N8N-010` | `n8n_workflows` | P0 | `PARTIAL` | After N8N-012 exact cleanup, implement and independently verify the Finance subtree without touching peer applications. |
| 19 | `N8N-003` | `n8n_workflows` | P0 | `PARTIAL` | After N8N-012 cleanup, run the guarded n8n restart and second-read proof without making hierarchy work depend on a new token-expiry wait. |
| 20 | `N8N-011` | `n8n_workflows` | P0 | `PARTIAL` | Review the 15-to-4 machine map and WF20 artifact gates; do not create generic receipt, period-close or provider-circuit tables. |
| 21 | `N8N-005` | `n8n_workflows` | P0 | `IMPLEMENTED_UNVERIFIED` | Run concurrent and kill-at-boundary disposable tests for the fenced Actual outbox and prove one row, one verified commit and one cursor advance. |
| 22 | `ACTUAL-019` | `actual_finance` | P0 | `BLOCKED` | Execute the current corpus twice through the inactive n8n writer. |
| 23 | `PLATFORM-004` | `platform_operations` | P0 | `PARTIAL` | Record immutable image digests, verify independent restarts, execute restore drill, validate tunnel origin/headers and AD/Service Auth boundaries, and deploy reviewed n8n candidate. |
| 24 | `AUTO-001` | `automation_cutover` | P0 | `PARTIAL` | Prove one issuer schedule end to end while inactive. |
| 25 | `AUTO-003` | `automation_cutover` | P1 | `PARTIAL` | Complete n8n schedule runtime proof, cut over atomically, disable legacy Codex schedules, and confirm successful runs create durable receipts without new chats. |
| 26 | `ACTUAL-001` | `actual_finance` | P0 | `PARTIAL` | Enumerate every posted identity across Actual and operational stores and prove Actual is the sole posted ledger. |
| 27 | `ACTUAL-002` | `actual_finance` | P0 | `PARTIAL` | Capture fresh Sarwa holdings and FX before any guarded Actual account change. |
| 28 | `ACTUAL-004` | `actual_finance` | P0 | `PARTIAL` | Run each in-scope provider through immutable archive, n8n validation, reviewed delta, fenced writer, and Actual readback; add representative RAK/SC statement fixtures. |
| 29 | `ACTUAL-005` | `actual_finance` | P0 | `PARTIAL` | Populate only after account/UI parity; obtain approved budget values and verify every named report against authoritative source data. |
| 30 | `ACTUAL-017` | `actual_finance` | P0 | `PARTIAL` | Acquire fresh interactive holdings and FX, run disposable idempotent projection, then guardedly create/read back each included off-budget portfolio account. |
| 31 | `AGENT-001` | `agent_runtime` | P0 | `PARTIAL` | Execute the deterministic-first n8n path and prove AI is invoked only for explicitly unresolved fields. |
| 32 | `AUTO-002` | `automation_cutover` | P0 | `PARTIAL` | After OAuth binding, execute bounded expected-cycle polling and retain cursor, no-statement, and statement-found receipts. |
| 33 | `BROWSER-001` | `browser_acquisition` | P0 | `PARTIAL` | Run each in-scope provider through immutable archive, n8n validation, reviewed delta, fenced writer, and Actual readback; add representative RAK/SC statement fixtures. |
| 34 | `CASHBACK-001` | `cashback_companion` | P0 | `PARTIAL` | Enumerate every posted identity across Actual and operational stores and prove Actual is the sole posted ledger. |
| 35 | `CASHBACK-003` | `cashback_companion` | P0 | `PARTIAL` | Recheck public/mobile health, push triggers, weekly pace, period history/reset, fictional profiles, and n8n-fed live events after deployment cutover. |
| 36 | `CASHBACK-004` | `cashback_companion` | P0 | `PARTIAL` | Recheck public/mobile health, push triggers, weekly pace, period history/reset, fictional profiles, and n8n-fed live events after deployment cutover. |
| 37 | `CASHBACK-005` | `cashback_companion` | P0 | `PARTIAL` | Recheck public/mobile health, push triggers, weekly pace, period history/reset, fictional profiles, and n8n-fed live events after deployment cutover. |
| 38 | `CASHBACK-006` | `cashback_companion` | P0 | `PARTIAL` | Recheck public/mobile health, push triggers, weekly pace, period history/reset, fictional profiles, and n8n-fed live events after deployment cutover. |
| 39 | `CASHBACK-007` | `cashback_companion` | P0 | `IMPLEMENTED_UNVERIFIED` | Recheck public/mobile health, push triggers, weekly pace, period history/reset, fictional profiles, and n8n-fed live events after deployment cutover. |
| 40 | `CASHBACK-012` | `cashback_companion` | P0 | `PARTIAL` | Recheck public/mobile health, push triggers, weekly pace, period history/reset, fictional profiles, and n8n-fed live events after deployment cutover. |
| 41 | `CASHBACK-013` | `cashback_companion` | P0 | `PARTIAL` | Recheck public/mobile health, push triggers, weekly pace, period history/reset, fictional profiles, and n8n-fed live events after deployment cutover. |
| 42 | `CASHBACK-014` | `cashback_companion` | P0 | `PARTIAL` | Finish disposable semantic replay before live cashback acceptance. |
| 43 | `N8N-004` | `n8n_workflows` | P0 | `PARTIAL` | Run the sole writer in disposable Actual and compare every economic field and balance. |
| 44 | `N8N-006` | `n8n_workflows` | P0 | `IMPLEMENTED_UNVERIFIED` | Keep all workflows inactive while completing disposable double replay and reviewed finance-write acceptance. |
| 45 | `PLATFORM-001` | `platform_operations` | P0 | `PARTIAL` | Run the independent restart, protected-route, and restore acceptance matrix. |
| 46 | `ACTUAL-007` | `actual_finance` | P1 | `PARTIAL` | Run classification audit over the full disposable corpus and authenticated production readback; resolve owner/property/payee exceptions. |
| 47 | `ACTUAL-008` | `actual_finance` | P1 | `PARTIAL` | Run classification audit over the full disposable corpus and authenticated production readback; resolve owner/property/payee exceptions. |
| 48 | `ACTUAL-009` | `actual_finance` | P1 | `PARTIAL` | Run classification audit over the full disposable corpus and authenticated production readback; resolve owner/property/payee exceptions. |
| 49 | `ACTUAL-012` | `actual_finance` | P1 | `PARTIAL` | Run classification audit over the full disposable corpus and authenticated production readback; resolve owner/property/payee exceptions. |
| 50 | `ACTUAL-013` | `actual_finance` | P1 | `PARTIAL` | Populate only after account/UI parity; obtain approved budget values and verify every named report against authoritative source data. |
| 51 | `ACTUAL-014` | `actual_finance` | P1 | `PARTIAL` | Populate only after account/UI parity; obtain approved budget values and verify every named report against authoritative source data. |
| 52 | `ACTUAL-016` | `actual_finance` | P1 | `PARTIAL` | Populate only after account/UI parity; obtain approved budget values and verify every named report against authoritative source data. |
| 53 | `ACTUAL-018` | `actual_finance` | P1 | `IMPLEMENTED_UNVERIFIED` | Populate only after account/UI parity; obtain approved budget values and verify every named report against authoritative source data. |
| 54 | `ACTUAL-020` | `actual_finance` | P1 | `PARTIAL` | Run classification audit over the full disposable corpus and authenticated production readback; resolve owner/property/payee exceptions. |
| 55 | `AGENT-002` | `agent_runtime` | P1 | `PARTIAL` | Prove server-owned normal and exception model routing, including the gated high-reasoning path, in disposable n8n. |
| 56 | `BROWSER-004` | `browser_acquisition` | P1 | `PARTIAL` | Run each in-scope provider through immutable archive, n8n validation, reviewed delta, fenced writer, and Actual readback; add representative RAK/SC statement fixtures. |
| 57 | `CASHBACK-002` | `cashback_companion` | P1 | `PARTIAL` | Recheck public/mobile health, push triggers, weekly pace, period history/reset, fictional profiles, and n8n-fed live events after deployment cutover. |
| 58 | `CASHBACK-008` | `cashback_companion` | P1 | `IMPLEMENTED_UNVERIFIED` | Recheck public/mobile health, push triggers, weekly pace, period history/reset, fictional profiles, and n8n-fed live events after deployment cutover. |
| 59 | `CASHBACK-009` | `cashback_companion` | P1 | `PARTIAL` | Recheck public/mobile health, push triggers, weekly pace, period history/reset, fictional profiles, and n8n-fed live events after deployment cutover. |
| 60 | `CASHBACK-010` | `cashback_companion` | P1 | `PARTIAL` | Recheck public/mobile health, push triggers, weekly pace, period history/reset, fictional profiles, and n8n-fed live events after deployment cutover. |
| 61 | `CASHBACK-011` | `cashback_companion` | P1 | `IMPLEMENTED_UNVERIFIED` | Recheck public/mobile health, push triggers, weekly pace, period history/reset, fictional profiles, and n8n-fed live events after deployment cutover. |
| 62 | `DOC-001` | `document_evidence` | P1 | `PARTIAL` | Import and run the inactive setup-only workflow once, retain the redacted exact-root receipt, then proceed with archive normalization. |
...
| ID | Owner | Strict dependencies | Related tasks | Status | Acceptance evidence and validator | Title |
| --- | --- | --- | --- | --- | --- | --- |
| `DOC-001` | `document_evidence` | `DOC-007`, `PLATFORM-003`, `N8N-012` | None | `PARTIAL` | PARTIAL / REPOSITORY_TESTED_ROOT_SETUP_LIVE_RUN_PENDING; 7 files, 3 tests; live readback: recorded; 4 acceptance checks; validator: evidence tests plus immutable archive and readback receipt review | Immutable OneDrive evidence archive |
...
| `N8N-002` | `n8n_workflows` | `N8N-001`, `N8N-008` | None | `PARTIAL` | DEPLOYED_INACTIVE / RUNTIME_SIBLING_PLACEMENT_VERIFIED_TARGET_APPLICATION_HIERARCHY_PENDING; 7 files, 3 tests; live readback: recorded; 4 acceptance checks; validator: n8n contract tests plus exact disposable execution receipt review | Workflow stages remain visible and composable |
...
| `N8N-010` | `n8n_workflows` | `N8N-012` | `N8N-002`, `N8N-003`, `N8N-006`, `N8N-011`, `DOC-001`, `PLATFORM-001` | `PARTIAL` | DESIGN_RECORDED_UNIMPLEMENTED / DESIGN_RECORDED_UNIMPLEMENTED; 3 files, 1 tests; live readback: recorded; 6 acceptance checks; validator: n8n contract tests plus exact disposable execution receipt review | Application-scoped role hierarchy |
| `N8N-011` | `n8n_workflows` | None | `N8N-004`, `N8N-005`, `N8N-007`, `N8N-009`, `AGENT-003`, `DOC-007`, `PLATFORM-006` | `PARTIAL` | PARTIAL / FIFTEEN_TABLE_BASELINE_VERIFIED_MINIMALITY_AND_MIGRATION_UNPROVEN; 5 files, 2 tests; live readback: recorded; 11 acceptance checks; validator: n8n contract tests plus exact disposable execution receipt review | Minimize Data Tables to durable finance-domain state |
...
| ID | Owner | Strict dependencies | Related tasks | Status | Acceptance evidence and validator | Title |
| --- | --- | --- | --- | --- | --- | --- |
| `AUTO-001` | `automation_cutover` | `CASHBACK-005`, `CASHBACK-006`, `N8N-001` | None | `PARTIAL` | PARTIAL / INACTIVE_N8N_IMPORT_VERIFIED_CUTOVER_PENDING; 7 files, 3 tests; live readback: recorded; 4 acceptance checks; validator: automation lifecycle tests plus target scheduler and legacy-task readback | Issuer-specific production schedule matrix |
