...
def source_snapshot() -> dict[str, str]:
    ...
    return {
        ...
        "node_scan_digest_method": (
            "From the finance repository root, hash each JSON file under scan_roots "
            "with sha256sum, sort the complete hash-and-relative-path lines bytewise, "
            "then hash that stream with sha256sum."
        ),
    }
