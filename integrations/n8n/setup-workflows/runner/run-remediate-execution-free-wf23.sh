...
restore_n8n_if_stopped() {
  if [[ "${n8n_stopped:-false}" == "true" ]]; then
    if [[ "$(docker inspect -f '{{.State.Running}}' "${n8n_container}" 2>/dev/null || true)" != "true" ]]; then
