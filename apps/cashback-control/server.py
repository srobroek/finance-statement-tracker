...
def historical_periods(
    ...
    if limit <= 0:
        return []
    configured_programs = programs_from_config(
        load_program_configuration(PROGRAM_CONFIG_PATH)
    )
    eligible_cards = eligible_card_codes(configured_programs, effective_memberships)
    programs_by_card = {
        program.card.upper(): program for program in configured_programs
    }
    ...
    for event in event_rows:
        ...
        program = programs_by_card.get(card_code)
    ...
    for (card_code, cycle_start, cycle_end), event_times in event_periods.items():
        ...
        effective_start = getattr(programs_by_card[card_code], "effective_start", None)
