...
def fingerprint_attachment(message_id: str, attachment: Mapping[str, Any]) -> dict[str, Any]:
    ...
    content = _decode_b64(attachment.get("content_base64"), f"ATTACHMENT_CONTENT:{attachment_id}")
...
def build_source_bindings(root: Path | None = None) -> dict[str, Any]:
    ...
    for relative in DEFAULT_SOURCE_PATHS + DEFAULT_CONFIG_PATHS + (
        "integrations/n8n/application-images.lock.json",
    ) + DEFAULT_WORKFLOW_PATHS:
        path = base / relative
        if not path.is_file():
            raise ContractError(f"SOURCE_ARTIFACT_MISSING:{relative}")
    ...
    image_lock = base / "integrations/n8n/application-images.lock.json"
...
def _make_receipt(
    ...
    *,
    ...
    receipt: dict[str, Any] = {
        ...
        "enumeration": {
            "workflow_chain": ["W12", "W01", "W22", "W17", "W03", "W20"],
...
def verify_receipt(receipt: Mapping[str, Any], *, root: Path | None = None) -> dict[str, Any]:
    ...
    for field_name in ("source_sha256", "config_sha256"):
        _hex_digest(bindings[field_name], field_name)
    image = str(bindings["image_digest"])
    if not image.startswith("sha256:"):
        raise ContractError("IMAGE_DIGEST_INVALID")
    _hex_digest(image.removeprefix("sha256:"), "IMAGE")
    for path, digest in bindings["workflow_sha256"].items():
        _hex_digest(digest, f"WORKFLOW:{path}")
    if root is not None:
        base = Path(root).resolve()
        for artifact in bindings["source_artifacts"]:
            path = base / artifact["path"]
            if not path.is_file():
                raise ContractError(f"SOURCE_ARTIFACT_MISSING:{artifact['path']}")
            observed = sha256_bytes(path.read_bytes().replace(b"\r\n", b"\n"))
            if observed != artifact["sha256"]:
                raise ContractError(f"SOURCE_ARTIFACT_SHA256_MISMATCH:{artifact['path']}")
    enumeration = value["enumeration"]
    if not enumeration["enumerated_exactly_once"]:
        raise ContractError("ENUMERATION_NOT_EXACTLY_ONCE")
    if enumeration["scanned_count"] != len(enumeration["ordered_message_ids"]):
        raise ContractError("ENUMERATION_COUNT_MISMATCH")
    ...
    if not (
        ...
        and actual["economic_readback_sha256"] == actual["api_readback_sha256"] == actual["ui_readback_sha256"]
    ):
        raise ContractError("ACTUAL_ECONOMIC_READBACK_MISMATCH")
    cashback = value["cashback"]
    if cashback["status"] not in {"VERIFIED", "N/A"}:
        raise ContractError("CASHBACK_READBACK_STATUS_INVALID")
    cursor = value["cursor_cas"]
    if not cursor["readback_verified"] or cursor["after_version"] != 1:
        raise ContractError("CURSOR_CAS_READBACK_INVALID")
    replay = value["replay"]
    if not replay["idempotent"] or any(
        replay[key] != 0
        for key in (
            "archive_new_writes",
            "attachment_new_writes",
            "actual_new_writes",
            "cashback_new_writes",
            "cursor_new_writes",
        )
    ...
    if not value["restart"]["all_recovered"] or len(value["restart"]["injections"]) != len(RESTART_INJECTION_POINTS):
        raise ContractError("RESTART_MATRIX_INCOMPLETE")
    if value["cleanup"]["status"] != "CLEAN" or value["cleanup"]["production_writes"] != 0:
...
def _main() -> int:
    ...
    if args.count < 0 or args.count > 101:
        parser.error("--count must be between 0 and 101")
