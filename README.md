...
Five Codex automations are active: one twice-daily live transaction scan and four source-specific monthly statement jobs for RAKBANK World, Standard Chartered Platinum X, Emirates Islamic Amazon, and Wio. RAKBANK and Standard Chartered close on day 5 and run on day 6; Emirates Islamic and Wio process the prior calendar month on day 1. The ADCB statement task was removed and the former daily aggregate gate is paused. Every job is idempotent and leaves cursors, Actual, or close state unchanged when a required connector, source contract, parser, account mapping, or evidence gate is unavailable. The live Luna/max job scans the complete durable-cursor gap plus overlap and submits exact RAKBANK Outlook transaction-message objects through `push-outlook-messages.ps1`; SC, EI, and Wio source recipes remain dormant until their real notification contracts are captured and tested. The continuous companion performs deterministic parsing, LIVE_CASHBACK rules, deduplication, persistence, and dashboard refresh in one acknowledged call. Evidence search and full-budget classification run only in the monthly statement jobs. Failed payloads remain under `runtime` for retry.
