...
50,000.00 49,990.00 10.00 25/08/26 10.00 0.00 10.00`, 'emirates_islamic_v1');
  ...
  const checked = assertPreparedOutbox({ schema_version: 1, outbox_id: 'ei', state: 'PREPARED', account_id: 'ei-account', execution_context: { trigger: 'SUBWORKFLOW', manual: false, mcp: false }, writer_lease: { resource_key: 'actual:ei-account', lease_id: 'lease', fencing_token: 1, expires_at: new Date(Date.now() + 60_000).toISOString() }, transactions: rows });
