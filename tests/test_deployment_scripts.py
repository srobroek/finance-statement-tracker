...
class DeploymentScriptTests(unittest.TestCase):
    ...
    def test_actual_ingestion_upload_is_private_and_readable_by_worker(self) -> None:
        ...
        self.assertIn("$parsedAIResponses = ConvertFrom-Json -InputObject $rawAIResponses", script)
        self.assertIn("$parsedEvidenceLinks = ConvertFrom-Json -InputObject $rawEvidenceLinks", script)
        self.assertNotIn("@($rawAIResponses | ConvertFrom-Json)", script)
        self.assertNotIn("@($rawEvidenceLinks | ConvertFrom-Json)", script)
        self.assertIn("ai_handoff_complete = $AIHandoffComplete.IsPresent", script)
        self.assertIn("$response = @($payload | & ssh", script)
        self.assertIn("$OutputEncoding = [Text.UTF8Encoding]::new($false)", script)
        self.assertIn("$OutputEncoding = $previousOutputEncoding", script)
