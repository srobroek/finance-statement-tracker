...
| Durable operational state | Data Tables v4 define all 15 state tables, retention, logical identity, concurrency, and readback semantics | all tables connected test; bootstrap generator/currentness tests | Baseline created 15 tables; corrected seed replay pending |
...
| Foldered UI | `workflow-folders.json` maps all 21 workflows to eight folders; exports carry target metadata and native tags; placement SQL is inactive/project guarded | folder completeness/readback-guard test | Post-import disposable folder/API readback pending |
