...
class MaintenanceWorkflowTests(unittest.TestCase):
    @unittest.skipUnless(
        os.environ.get("N8N_NODE_MODULES"),
        "installed pinned n8n nodes require N8N_NODE_MODULES",
    )
    def test_native_receipt_nodes(self):
        result = subprocess.run(
            [
                "node",
                str(ROOT / "tests/maintenance-native-nodes.cjs"),
                os.environ["N8N_NODE_MODULES"],
                str(FILE),
            ],
            capture_output=True,
            text=True,
    ...
    def test_export_is_deterministic_inactive_and_uses_global_fence(self):
        spec = importlib.util.spec_from_file_location(
            ...
            ROOT / "integrations/n8n/generate_adcb_maintenance_workflow.py",
        )
        module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(module)
        w = json.loads(FILE.read_text())
        self.assertEqual(w, module.build())
        self.assertIs(w["active"], False)
        self.assertTrue(w["meta"]["publicationRequiresReview"])
        self.assertTrue(w["meta"]["scheduleForbidden"])
        self.assertNotIn("activationForbidden", w["meta"])
        self.assertTrue(w["meta"]["cursorMutationForbidden"])
        manifest = json.loads((FILE.parent / "manifest.json").read_text())
        entry = next(
            x for x in manifest["workflows"] if x["code"] == "ADCB_REVIEWED_MAINTENANCE"
        )
        self.assertTrue(entry["publication_requires_review"])
        self.assertTrue(entry["schedule_forbidden"])
        self.assertNotIn("activation_forbidden", entry)
        self.assertTrue(
            all(
                x.get("activation_forbidden") is True
                for x in manifest["workflows"]
                if x["code"] != "ADCB_REVIEWED_MAINTENANCE"
            )
        )
        self.assertFalse(
            any(
                n["type"].endswith(
                    (".manualTrigger", ".scheduleTrigger", ".webhook", ".dataTable")
                )
                for n in w["nodes"]
            )
        )
        ...
        mutation = [
            n for n in w["nodes"] if n["type"] == "n8n-nodes-finance.actualBudget"
        ]
        self.assertEqual(len(mutation), 1)
        self.assertEqual(
            mutation[0]["parameters"],
            {
        ...
        extract = next(
            n for n in w["nodes"] if n["name"] == "Extract Immutable Maintenance Plan"
        )
        self.assertEqual(extract["parameters"]["destinationKey"], "maintenance_plan")
        convert = next(
            n for n in w["nodes"] if n["name"] == "Convert Redacted Maintenance Receipt"
        )
        self.assertEqual(convert["parameters"]["mode"], "each")
        self.assertNotIn("sourceProperty", convert["parameters"])
        upload = next(
            n for n in w["nodes"] if n["name"] == "Archive Redacted Maintenance Receipt"
        )
        self.assertEqual(
            upload["typeVersion"], 1.1
        )  # v1 silently prefers binary filename
        self.assertIn("$execution.id", upload["parameters"]["fileName"])
        names = [n["name"] for n in w["nodes"]]
        for a, b in zip(names, names[1:]):
            self.assertEqual(
                w["connections"][a]["main"], [[{"node": b, "type": "main", "index": 0}]]
            )
        self.assertLess(
            names.index("Verify Approved Plan Bytes and Identity"),
            names.index("Apply Bounded Reviewed Maintenance"),
        )
        self.assertLess(
            names.index("Verify Durable Maintenance Receipt"),
            names.index("Release Exact Maintenance Fence"),
        )

    def test_real_code_rejects_wrong_plan_stale_fence_false_completion_and_archive_failure(
        self,
    ):
        script = r"""
...
const l={lease_class:'MAINTENANCE',resource_key:'actual:sync-1',lease_id:'12345678-1234-1234-1234-123456789abc',lease_owner:'n8n:maintenance:test-25',fencing_token:7,expires_at:new Date(Date.now()+600000).toISOString()};
const lr={...refs,'Acquire Global Actual Writer Fence':l};
const acquire=run('Build Global Maintenance Lease Request',{},refs);assert.equal(acquire.resource_key,l.resource_key);assert.equal(acquire.ttl_seconds,600);assert.equal(acquire.lease_class,'MAINTENANCE');
const assertion=run('Build Maintenance Fence Assert',{},lr);assert.equal(assertion.fencing_token,7);assert.equal(assertion.lease_class,'MAINTENANCE');
...
const release=run('Build Exact Maintenance Lease Release',artifact,lr);assert.equal(release.operation,'RELEASE');assert.equal(release.lease_class,'MAINTENANCE');
