...
class FxSnapshot:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ValueError("FX snapshot schema_version must be integer 1")
...
def validate_cashback_state(source: Mapping[str, Any]) -> None:
    ...
    receipt_sources = [(item.card_code, item.source_identity) for item in receipts]
    if len(set(receipt_sources)) != len(receipt_sources):
        raise ValueError(
            "Cashback state contains duplicate statement receipt source identities"
        )
    ...
    accounting_periods = [(item.card_code, item.period_id) for item in accounting]
    if len(set(accounting_periods)) != len(accounting_periods):
        raise ValueError(
            "Cashback state contains duplicate accounting rows for a card period"
        )
