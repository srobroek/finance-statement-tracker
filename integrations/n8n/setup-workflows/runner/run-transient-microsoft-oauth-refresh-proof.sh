...
for helper in bind-microsoft-oauth-refresh-proof.py validate_microsoft_oauth_refresh_evidence.py build_microsoft_oauth_failure_receipt.py n8n-cli-redacted-microsoft-oauth-refresh-proof.cjs n8n-cli-microsoft-oauth-metadata-readback.cjs n8n-cli-finance-data-table-digest.cjs n8n-cli-remove-transient-microsoft-oauth-refresh-proof.cjs; do
...
read_metadata() {
  ...
  printf '%s' "${raw}" | python3 -c 'import json,sys; p="microsoft oauth metadata readback verified:"; s=sys.stdin.read(); assert len(s.encode())<=65536 and s.count("\n")<=1 and s.startswith(p); v=json.loads(s[len(p):]); assert set(v["credentials"])=={"outlook","onedrive"} and v["status"]=="VERIFIED" and v["database_writes"] is False and v["provider_calls"] is False and v["credential_ids_recorded"] is False and v["secret_values_recorded"] is False and v["token_fingerprints_recorded"] is False; [(_ for _ in ()).throw(AssertionError()) for x in v["credentials"].values() if x["access_token_present"] is not True or x["refresh_token_present"] is not True or x["expiration_observed"] is not True]; print(json.dumps(v,separators=(",",":")))'
}
...
data_table_digest() {
  ...
  printf '%s' "${raw}" | python3 -c 'import json,re,sys; p="finance data table digest verified:"; s=sys.stdin.read(); assert len(s.encode())<=65536 and s.count("\n")<=1 and s.startswith(p); v=json.loads(s[len(p):]); assert v["status"]=="VERIFIED" and v["finance_tables"]==15 and re.fullmatch(r"[0-9a-f]{64}",v["digest_sha256"]) and v["writes_performed"] is False and v["provider_calls"] is False and v["row_values_recorded"] is False; print(v["digest_sha256"])'
}
