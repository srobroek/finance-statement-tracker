...
After statement normalization, every card-specific job performs a selective evidence pass over the closed period using `config/evidence-search-policy.json`. It searches for utility/property bills, warranty-bearing goods, significant online orders, travel bookings, contracts, and qualifying high-value purchases. Routine groceries, dining, delivery, coffee, fuel, parking/tolls, and ordinary transport are explicitly excluded unless manually requested. Matching uses at least two strong facts such as amount/currency, order/reference, normalized vendor, date buffer, and account identity; vendor similarity alone is insufficient. Confirmed original attachments are preferred; a confirmed evidence email without a relevant attachment is rendered to a PDF or image snapshot. The result is hash-deduplicated under `Finance Evidence/YYYY/MM/vendor-slug/`, recorded in `Finance Evidence/catalogue.json`, and linked through the transaction evidence metadata. Ambiguous candidates remain unmatched and review-required.
