        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            try:
                ...
                receipt_payload = json.dumps(
                    {
                        "source": "outlook",
                        "source_message_id": "api-bank-message",
                        "received_at": "2026-09-06T00:00:00Z",
                        "card_code": "RAK_WORLD",
                    }
                ).encode("utf-8")
                receipt_request = urllib.request.Request(
                    f"http://127.0.0.1:{port}/api/statement-receipts",
                    data=receipt_payload,
                    headers={"Content-Type": "application/json"},
                    method="POST",
                )
                with self.assertRaises(urllib.error.HTTPError) as receipt_auth:
                    urllib.request.urlopen(receipt_request, timeout=1)
                self.assertEqual(receipt_auth.exception.code, 401)
                receipt_auth.exception.close()
                receipt_request.add_header("Authorization", "Bearer test-token")
                with urllib.request.urlopen(receipt_request, timeout=1) as response:
                    receipt_result = json.loads(response.read())
                self.assertEqual(
                    receipt_result["statement_receipt"]["state"], "BANK_CLOSED"
                )
                with urllib.request.urlopen(receipt_request, timeout=1) as response:
                    receipt_replay = json.loads(response.read())
                ...
                self.assertEqual(
                    correction["correction"]["source_event_id"], "api-test:1"
                )
                ...
                self.assertEqual(
                    reconciliation["reconciliation"]["notification_only"], 0
                )

                receipt = actual_receipt("synthetic-statement-2026-08-06--2026-09-05")
                finalized = post(
                    "periods/finalize",
                    {
                        "statement_reference": "synthetic-statement-2026-08-06--2026-09-05",
                ...
                self.assertEqual(
                    replayed["period"]["close_id"], finalized["period"]["close_id"]
                )
                missing_receipt = {
                    key: value
                    for key, value in {
                        ...
                        "messages": [
                            {
                                ...
                                "sender": {
                                    "emailAddress": {"address": "alerts@rakbank.ae"}
                                },
                ...
                self.assertIn(
                    ...
                    {
                        period["status"]
                        for period in dashboard["data_status"]["card_periods"]
                    },
                )
                with urllib.request.urlopen(
                ...
                self.assertEqual(
                    history["periods"][0]["summary"]["expected_cashback_aed"], "0.00"
                )
            finally:
                process.terminate()
                try:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            def start_server() -> subprocess.Popen[str]:
                started = subprocess.Popen(
                    [
                        sys.executable,
                        str(ROOT / "apps" / "cashback-control" / "server.py"),
                    ],
                    cwd=ROOT,
                    env=env,
                    stdout=subprocess.DEVNULL,
            ...
            try:
                ...
                self.assertEqual(
                    post("reconcile", reconciliation_payload)["reconciliation"][
                        "matched"
                    ],
                    0,
                )
                ...
                self.assertEqual(
                    periods["periods"][0]["statement_reference"], reference
                )
            finally:
                stop_server()
