...
class DeploymentScriptTests(unittest.TestCase):
    ...
    def test_cashback_deployment_uses_container_local_probes_and_exact_snapshot(self) -> None:
        workflow = Path(".github/workflows/cashback-image.yml").read_text(encoding="utf-8")
        ...
        self.assertIn("sudo python3 deploy/cashback/predeploy-backup.py", workflow)
        deploy = workflow.split("\n  deploy:\n", 1)[1]
        self.assertNotIn("sanitize-cashback-backup.py", deploy)
        self.assertIn("sudo docker stop finance-cashback-control", deploy)
        backup = Path("deploy/cashback/predeploy-backup.py").read_text(encoding="utf-8")
        self.assertIn("'secrets_included': True", backup)
        self.assertIn("live.backup(backup)", backup)
    ...
    def test_cashback_deployment_pins_published_digest_and_retains_rollback(self) -> None:
        ...
        self.assertIn('backup="/opt/backups/finance-cashback/${stamp}"', deploy)
        self.assertIn('"$CASHBACK_ROLLBACK_BACKUP/verification.json"', deploy)
        backup = Path("deploy/cashback/predeploy-backup.py").read_text(encoding="utf-8")
        self.assertIn("'finance-cashback/compose.yaml'", backup)
        self.assertIn("'finance-cashback/.env'", backup)
        self.assertIn("'rollback_image': image_reference", backup)
