...
## Preconditions
- P1: Exact reviewed image/config digests, target host, backup, and rollback package are bound.
- P2: The exclusive deployment lane and approval boundary are available; no concurrent writer is active.
...
- **Expect:** Health and topology match the reviewed contract.
- **Expect (negative):** No financial mutation occurred.
...
## Success criteria
- SC1: S1-S4: Acceptance is exact digest/topology parity, inactive/write-disabled state, and protected rollback proof.

## Known gaps
- G1: The authoritative journey corpus and deployment receipt are unavailable locally; source and tests are not production proof. Keep draft until current host evidence exists. Trace evidence: `AGENTS.md:29-36`, `README.md:88-99`, `integrations/n8n/`, `config/codex-automations.json`, Beads `orc-n2q.379.48`, and review `orc-n2q.379.100`.
