...
Progress recorded: **2026-08-20** at `4a30c7f`.
Evidence commits: `4a30c7f`, `8149f42`, `b07c410`, `00491aa`, `c8d4f7c`, `3a6acc6`, `da6b0c1`, `b845ccf`, `fa8fd58`
...
- The internal runtime health check passed for n8n, the task broker, task launcher, and bounded Codex runner. The transient WF23 run completed bounded Outlook and OneDrive reads and moved both expired access-token expiries into the future, but restart persistence and the required second reads did not run.
- WF23 is currently stranded as one inactive, unpublished setup workflow with one soft-deleted running execution row. The observed project boundary is temporarily 22 workflows, zero active, zero published, 22 placements, and 66 tag edges; exact reviewed cleanup back to 21/0/0 and zero WF23 rows is pending.
...
| 2 | `N8N-001` | `n8n_workflows` | P0 | `IMPLEMENTED_UNVERIFIED` | Remove the exact WF23 orphan, then complete the guarded Microsoft restart and second-read proof while all workflows remain inactive. |
| 3 | `N8N-002` | `n8n_workflows` | P1 | `PARTIAL` | Complete setup proofs and N8N-011 first, then implement N8N-010; use Tidy Workflow only for optional coordinate cleanup. |
...
| 17 | `N8N-003` | `n8n_workflows` | P0 | `PARTIAL` | Restore the exact 21/0/0 boundary, wait for both expiries to be past, and run the guarded restart proof once without recording credential identifiers or token values. |
| 18 | `N8N-011` | `n8n_workflows` | P0 | `PARTIAL` | Audit all 15 tables and every column, then review the candidate consolidation and relocation design before any implementation. |
| 19 | `N8N-010` | `n8n_workflows` | P1 | `PARTIAL` | Wait for the setup and N8N-011 gates, then implement and independently verify the Finance subtree without touching peer applications. |
| 20 | `N8N-005` | `n8n_workflows` | P0 | `IMPLEMENTED_UNVERIFIED` | Run concurrent and kill-at-boundary disposable tests for the fenced Actual outbox and prove one row, one verified commit and one cursor advance. |
| 21 | `ACTUAL-019` | `actual_finance` | P0 | `BLOCKED` | Execute the current corpus twice through the inactive n8n writer. |
| 22 | `PLATFORM-004` | `platform_operations` | P0 | `PARTIAL` | Record immutable image digests, verify independent restarts, execute restore drill, validate tunnel origin/headers and AD/Service Auth boundaries, and deploy reviewed n8n candidate. |
| 23 | `AUTO-001` | `automation_cutover` | P0 | `PARTIAL` | Prove one issuer schedule end to end while inactive. |
| 24 | `AUTO-003` | `automation_cutover` | P1 | `PARTIAL` | Complete n8n schedule runtime proof, cut over atomically, disable legacy Codex schedules, and confirm successful runs create durable receipts without new chats. |
| 25 | `ACTUAL-001` | `actual_finance` | P0 | `PARTIAL` | Enumerate every posted identity across Actual and operational stores and prove Actual is the sole posted ledger. |
| 26 | `ACTUAL-002` | `actual_finance` | P0 | `PARTIAL` | Capture fresh Sarwa holdings and FX before any guarded Actual account change. |
| 27 | `ACTUAL-004` | `actual_finance` | P0 | `PARTIAL` | Run each in-scope provider through immutable archive, n8n validation, reviewed delta, fenced writer, and Actual readback; add representative RAK/SC statement fixtures. |
| 28 | `ACTUAL-005` | `actual_finance` | P0 | `PARTIAL` | Populate only after account/UI parity; obtain approved budget values and verify every named report against authoritative source data. |
| 29 | `ACTUAL-017` | `actual_finance` | P0 | `PARTIAL` | Acquire fresh interactive holdings and FX, run disposable idempotent projection, then guardedly create/read back each included off-budget portfolio account. |
| 30 | `AGENT-001` | `agent_runtime` | P0 | `PARTIAL` | Execute the deterministic-first n8n path and prove AI is invoked only for explicitly unresolved fields. |
| 31 | `AUTO-002` | `automation_cutover` | P0 | `PARTIAL` | After OAuth binding, execute bounded expected-cycle polling and retain cursor, no-statement, and statement-found receipts. |
| 32 | `BROWSER-001` | `browser_acquisition` | P0 | `PARTIAL` | Run each in-scope provider through immutable archive, n8n validation, reviewed delta, fenced writer, and Actual readback; add representative RAK/SC statement fixtures. |
| 33 | `CASHBACK-001` | `cashback_companion` | P0 | `PARTIAL` | Enumerate every posted identity across Actual and operational stores and prove Actual is the sole posted ledger. |
| 34 | `CASHBACK-003` | `cashback_companion` | P0 | `PARTIAL` | Recheck public/mobile health, push triggers, weekly pace, period history/reset, fictional profiles, and n8n-fed live events after deployment cutover. |
| 35 | `CASHBACK-004` | `cashback_companion` | P0 | `PARTIAL` | Recheck public/mobile health, push triggers, weekly pace, period history/reset, fictional profiles, and n8n-fed live events after deployment cutover. |
| 36 | `CASHBACK-005` | `cashback_companion` | P0 | `PARTIAL` | Recheck public/mobile health, push triggers, weekly pace, period history/reset, fictional profiles, and n8n-fed live events after deployment cutover. |
| 37 | `CASHBACK-006` | `cashback_companion` | P0 | `PARTIAL` | Recheck public/mobile health, push triggers, weekly pace, period history/reset, fictional profiles, and n8n-fed live events after deployment cutover. |
| 38 | `CASHBACK-007` | `cashback_companion` | P0 | `IMPLEMENTED_UNVERIFIED` | Recheck public/mobile health, push triggers, weekly pace, period history/reset, fictional profiles, and n8n-fed live events after deployment cutover. |
| 39 | `CASHBACK-012` | `cashback_companion` | P0 | `PARTIAL` | Recheck public/mobile health, push triggers, weekly pace, period history/reset, fictional profiles, and n8n-fed live events after deployment cutover. |
| 40 | `CASHBACK-013` | `cashback_companion` | P0 | `PARTIAL` | Recheck public/mobile health, push triggers, weekly pace, period history/reset, fictional profiles, and n8n-fed live events after deployment cutover. |
| 41 | `CASHBACK-014` | `cashback_companion` | P0 | `PARTIAL` | Finish disposable semantic replay before live cashback acceptance. |
| 42 | `N8N-004` | `n8n_workflows` | P0 | `PARTIAL` | Run the sole writer in disposable Actual and compare every economic field and balance. |
| 43 | `N8N-006` | `n8n_workflows` | P0 | `IMPLEMENTED_UNVERIFIED` | Keep all workflows inactive while completing disposable double replay and reviewed finance-write acceptance. |
| 44 | `PLATFORM-001` | `platform_operations` | P0 | `PARTIAL` | Run the independent restart, protected-route, and restore acceptance matrix. |
| 45 | `ACTUAL-007` | `actual_finance` | P1 | `PARTIAL` | Run classification audit over the full disposable corpus and authenticated production readback; resolve owner/property/payee exceptions. |
| 46 | `ACTUAL-008` | `actual_finance` | P1 | `PARTIAL` | Run classification audit over the full disposable corpus and authenticated production readback; resolve owner/property/payee exceptions. |
| 47 | `ACTUAL-009` | `actual_finance` | P1 | `PARTIAL` | Run classification audit over the full disposable corpus and authenticated production readback; resolve owner/property/payee exceptions. |
| 48 | `ACTUAL-012` | `actual_finance` | P1 | `PARTIAL` | Run classification audit over the full disposable corpus and authenticated production readback; resolve owner/property/payee exceptions. |
| 49 | `ACTUAL-013` | `actual_finance` | P1 | `PARTIAL` | Populate only after account/UI parity; obtain approved budget values and verify every named report against authoritative source data. |
| 50 | `ACTUAL-014` | `actual_finance` | P1 | `PARTIAL` | Populate only after account/UI parity; obtain approved budget values and verify every named report against authoritative source data. |
| 51 | `ACTUAL-016` | `actual_finance` | P1 | `PARTIAL` | Populate only after account/UI parity; obtain approved budget values and verify every named report against authoritative source data. |
| 52 | `ACTUAL-018` | `actual_finance` | P1 | `IMPLEMENTED_UNVERIFIED` | Populate only after account/UI parity; obtain approved budget values and verify every named report against authoritative source data. |
| 53 | `ACTUAL-020` | `actual_finance` | P1 | `PARTIAL` | Run classification audit over the full disposable corpus and authenticated production readback; resolve owner/property/payee exceptions. |
| 54 | `AGENT-002` | `agent_runtime` | P1 | `PARTIAL` | Prove server-owned normal and exception model routing, including the gated high-reasoning path, in disposable n8n. |
| 55 | `BROWSER-004` | `browser_acquisition` | P1 | `PARTIAL` | Run each in-scope provider through immutable archive, n8n validation, reviewed delta, fenced writer, and Actual readback; add representative RAK/SC statement fixtures. |
| 56 | `CASHBACK-002` | `cashback_companion` | P1 | `PARTIAL` | Recheck public/mobile health, push triggers, weekly pace, period history/reset, fictional profiles, and n8n-fed live events after deployment cutover. |
| 57 | `CASHBACK-008` | `cashback_companion` | P1 | `IMPLEMENTED_UNVERIFIED` | Recheck public/mobile health, push triggers, weekly pace, period history/reset, fictional profiles, and n8n-fed live events after deployment cutover. |
| 58 | `CASHBACK-009` | `cashback_companion` | P1 | `PARTIAL` | Recheck public/mobile health, push triggers, weekly pace, period history/reset, fictional profiles, and n8n-fed live events after deployment cutover. |
| 59 | `CASHBACK-010` | `cashback_companion` | P1 | `PARTIAL` | Recheck public/mobile health, push triggers, weekly pace, period history/reset, fictional profiles, and n8n-fed live events after deployment cutover. |
| 60 | `CASHBACK-011` | `cashback_companion` | P1 | `IMPLEMENTED_UNVERIFIED` | Recheck public/mobile health, push triggers, weekly pace, period history/reset, fictional profiles, and n8n-fed live events after deployment cutover. |
| 61 | `DOC-001` | `document_evidence` | P1 | `PARTIAL` | Import and run the inactive setup-only workflow once, retain the redacted exact-root receipt, then proceed with archive normalization. |
| 62 | `DOC-003` | `document_evidence` | P1 | `PARTIAL` | Exercise plain/encrypted live documents, verify ephemeral plaintext deletion, catalogue hashes, selective store/do-not-store policy, and transaction linkage. |
| 63 | `DOC-004` | `document_evidence` | P1 | `PARTIAL` | Review the 28 preserved manual-category conflicts and 15 corpus identities absent from the snapshot, replay the guarded plan in disposable Actual, then separately authorize any production delta and verify authenticated UI samples. |
| 64 | `DOC-005` | `document_evidence` | P1 | `PARTIAL` | Exercise plain/encrypted live documents, verify ephemeral plaintext deletion, catalogue hashes, selective store/do-not-store policy, and transaction linkage. |
| 65 | `DOC-006` | `document_evidence` | P1 | `PARTIAL` | Run each in-scope provider through immutable archive, n8n validation, reviewed delta, fenced writer, and Actual readback; add representative RAK/SC statement fixtures. |
| 66 | `DOCS-001` | `documentation_acceptance` | P1 | `PARTIAL` | Implement and prove: Fresh clone runbook. |
| 67 | `N8N-007` | `n8n_workflows` | P1 | `PARTIAL` | Run the disposable operations and error-workflow negative matrix and retain redacted durable receipts. |
| 68 | `PLATFORM-002` | `platform_operations` | P1 | `IMPLEMENTED_UNVERIFIED` | Run exact image build and provenance attestation in CI and compare the deployed digest. |
| 69 | `PLATFORM-005` | `platform_operations` | P1 | `IMPLEMENTED_UNVERIFIED` | Record immutable image digests, verify independent restarts, execute restore drill, validate tunnel origin/headers and AD/Service Auth boundaries, and deploy reviewed n8n candidate. |
| 70 | `ACTUAL-015` | `actual_finance` | P2 | `PARTIAL` | Populate only after account/UI parity; obtain approved budget values and verify every named report against authoritative source data. |
| 71 | `AGENT-004` | `agent_runtime` | P2 | `PARTIAL` | Run protected-field and automatic-write attacks and prove every AI result remains a review-only proposal. |
| 72 | `AGENT-005` | `agent_runtime` | P2 | `PARTIAL` | Prove identical provider boundaries in the disposable runtime. |
| 73 | `DOCS-003` | `documentation_acceptance` | P2 | `NOT_STARTED` | Implement and prove: Public-safe examples. |
| 74 | `N8N-009` | `n8n_workflows` | P2 | `IMPLEMENTED_UNVERIFIED` | Run allowed and malicious MCP facade requests with instance MCP disabled and prove zero unauthorized writes. |
| 75 | `PLATFORM-007` | `platform_operations` | P2 | `PARTIAL` | Implement and prove: Private repo visibility verified. |
...
| --- | --- | --- | --- | --- | --- | --- |
| `AGENT-001` | `agent_runtime` | `ACTUAL-005`, `ACTUAL-010` | None | `PARTIAL` | PARTIAL / FAILING_WORKTREE; 3 files, 0 tests; live readback: none; 4 acceptance checks; validator: runner contract tests plus isolated structured-output receipt review | AI runs only after deterministic rules and history matching |
| `AGENT-002` | `agent_runtime` | `AGENT-003` | None | `PARTIAL` | PARTIAL / FAILING_WORKTREE; 3 files, 0 tests; live readback: none; 4 acceptance checks; validator: runner contract tests plus isolated structured-output receipt review | Luna normal path and gated Sol exception path |
| `AGENT-003` | `agent_runtime` | `PLATFORM-003`, `N8N-007` | None | `PARTIAL` | PARTIAL / HOST_RUNNER_HEALTH_VERIFIED_PROVIDER_AUTH_BLOCKED; 7 files, 4 tests; live readback: recorded; 4 acceptance checks; validator: runner contract tests plus isolated structured-output receipt review | Ephemeral bounded Codex proposal runner |
...
| `ACTUAL-003` | `actual_finance` | `ACTUAL-002`, `PLATFORM-006` | `BROWSER-002`, `BROWSER-003` | `BLOCKED` | PARTIAL / TESTS_GREEN_LIVE_UI_API_PARITY_BLOCKED; 11 files, 7 tests; live readback: recorded; 4 acceptance checks; validator: full Python suite plus disposable Actual and authenticated UI/API readback | Explainable balances, net worth, and UI/API parity |
...
| ID | Owner | Strict dependencies | Related tasks | Status | Acceptance evidence and validator | Title |
| --- | --- | --- | --- | --- | --- | --- |
| `DOC-001` | `document_evidence` | `DOC-007`, `PLATFORM-003` | None | `PARTIAL` | PARTIAL / REPOSITORY_TESTED_ROOT_SETUP_LIVE_RUN_PENDING; 7 files, 3 tests; live readback: recorded; 4 acceptance checks; validator: evidence tests plus immutable archive and readback receipt review | Immutable OneDrive evidence archive |
...
| `N8N-002` | `n8n_workflows` | `N8N-001`, `N8N-008` | None | `PARTIAL` | DEPLOYED_INACTIVE / RUNTIME_SIBLING_PLACEMENT_VERIFIED_TARGET_APPLICATION_HIERARCHY_PENDING; 7 files, 2 tests; live readback: recorded; 4 acceptance checks; validator: n8n contract tests plus exact disposable execution receipt review | Workflow stages remain visible and composable |
| `N8N-003` | `n8n_workflows` | `N8N-004`, `DOC-007` | None | `PARTIAL` | DEPLOYED_INACTIVE / MICROSOFT_FIRST_READ_AND_REFRESH_OBSERVED_RESTART_PERSISTENCE_PENDING; 7 files, 2 tests; live readback: recorded; 4 acceptance checks; validator: n8n contract tests plus exact disposable execution receipt review | Cursor-safe Outlook acquisition |
| `N8N-004` | `n8n_workflows` | None | `ACTUAL-001`, `CASHBACK-001` | `PARTIAL` | PARTIAL / RUNTIME_EXACT_SCHEMA_AND_SEED_READBACK_VERIFIED_ACTUAL_READBACK_PENDING; 9 files, 3 tests; live readback: recorded; 4 acceptance checks; validator: n8n contract tests plus exact disposable execution receipt review | Postgres and Data Tables hold operational state only |
...
| `N8N-006` | `n8n_workflows` | `N8N-005`, `PLATFORM-006` | `N8N-001` | `IMPLEMENTED_UNVERIFIED` | DEPLOYED_INACTIVE / REGULAR_CORPUS_INACTIVE_VERIFIED_TRANSIENT_WF23_CLEANUP_PENDING; 9 files, 4 tests; live readback: recorded; 4 acceptance checks; validator: n8n contract tests plus exact disposable execution receipt review | Production workflows start inactive and write-disabled |
| `N8N-007` | `n8n_workflows` | `N8N-003`, `DOC-007`, `PLATFORM-003` | None | `PARTIAL` | PARTIAL / FAILING_WORKTREE; 4 files, 0 tests; live readback: none; 4 acceptance checks; validator: n8n contract tests plus exact disposable execution receipt review | Bounded operations and error workflows |
| `N8N-008` | `n8n_workflows` | `PLATFORM-002` | None | `IMPLEMENTED_UNVERIFIED` | DEPLOYED_INACTIVE / HOST_REGISTRATION_VERIFIED_AUTH_EXECUTION_PENDING; 8 files, 2 tests; live readback: recorded; 4 acceptance checks; validator: n8n contract tests plus exact disposable execution receipt review | Native-node-first and reviewed extension policy |
| `N8N-009` | `n8n_workflows` | `PLATFORM-004`, `N8N-007` | None | `IMPLEMENTED_UNVERIFIED` | PARTIAL / FAILING_WORKTREE; 4 files, 0 tests; live readback: none; 4 acceptance checks; validator: n8n contract tests plus exact disposable execution receipt review | Bounded MCP facade |
| `N8N-010` | `n8n_workflows` | `N8N-011`, `N8N-006` | `N8N-002`, `N8N-003`, `DOC-001`, `PLATFORM-001` | `PARTIAL` | PARTIAL / CURRENT_SIBLING_LAYOUT_VERIFIED_TARGET_FINANCE_HIERARCHY_UNIMPLEMENTED; 3 files, 1 tests; live readback: recorded; 6 acceptance checks; validator: n8n contract tests plus exact disposable execution receipt review | Application-scoped role hierarchy |
| `N8N-011` | `n8n_workflows` | None | `N8N-004`, `N8N-005`, `N8N-007`, `N8N-009`, `AGENT-003`, `DOC-007`, `PLATFORM-006` | `PARTIAL` | PARTIAL / FIFTEEN_TABLE_BASELINE_VERIFIED_MINIMALITY_AND_MIGRATION_UNPROVEN; 3 files, 1 tests; live readback: recorded; 8 acceptance checks; validator: n8n contract tests plus exact disposable execution receipt review | Minimize Data Tables to durable finance-domain state |
...
| `PLATFORM-006` | `platform_operations` | `PLATFORM-001`, `PLATFORM-003` | None | `PARTIAL` | PARTIAL / HOST_INACTIVE_DEPLOY_AND_HEALTH_VERIFIED_RESTORE_PENDING; 12 files, 7 tests; live readback: recorded; 5 acceptance checks; validator: platform tests plus image, deploy, restart, security, and restore receipt review | Backups, restore drills, restart health and deployment receipts |
...
| ID | Owner | Strict dependencies | Related tasks | Status | Acceptance evidence and validator | Title |
| --- | --- | --- | --- | --- | --- | --- |
| `AUTO-001` | `automation_cutover` | `CASHBACK-005`, `CASHBACK-006`, `N8N-001` | None | `PARTIAL` | PARTIAL / INACTIVE_N8N_IMPORT_VERIFIED_CUTOVER_PENDING; 7 files, 2 tests; live readback: recorded; 4 acceptance checks; validator: automation lifecycle tests plus target scheduler and legacy-task readback | Issuer-specific production schedule matrix |
