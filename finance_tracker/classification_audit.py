...
def build_actual_snapshot_classification_report(
    ...
    for row in raw_rows:
        ...
        # Older ingestion fixtures may omit all classification columns. They
        # cannot prove classification state, so leave them to field-shape
        # validation; Actual exporter rows always carry category/payee/tags.
        if not any(
            key in row
