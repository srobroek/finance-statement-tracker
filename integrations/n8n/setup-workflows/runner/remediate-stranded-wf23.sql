...
BEGIN
  ...
  IF (SELECT count(*) FROM workflow_entity w
    ...
    RAISE EXCEPTION 'WF23_REMEDIATION_WORKFLOW_CONTRACT_MISMATCH';
  END IF;
  ...
  IF jsonb_typeof(runtime_bindings) IS DISTINCT FROM 'array'
    ...
    RAISE EXCEPTION 'WF23_REMEDIATION_BINDER_ANNOTATION_MISMATCH';
  END IF;
  IF (SELECT count(*)
  ...
  IF live_workflow IS DISTINCT FROM scope.expected_workflow THEN
    RAISE EXCEPTION 'WF23_REMEDIATION_CANONICAL_WORKFLOW_MISMATCH';
  END IF;
  ...
  IF (SELECT count(*) FROM folder f WHERE f.id = scope.folder_id
    ...
    RAISE EXCEPTION 'WF23_REMEDIATION_FOLDER_MISMATCH';
  END IF;
  ...
  IF (SELECT count(*) FROM shared_workflow s WHERE s."workflowId" = scope.workflow_id
    ...
    RAISE EXCEPTION 'WF23_REMEDIATION_SHARE_MISMATCH';
  END IF;
  ...
  IF (SELECT string_agg(t.name, ',' ORDER BY t.name)
    ...
    RAISE EXCEPTION 'WF23_REMEDIATION_TAG_MISMATCH';
  END IF;
  ...
  IF (SELECT count(*) FROM workflow_entity other
    ...
    RAISE EXCEPTION 'WF23_REMEDIATION_WORKFLOW_DEPENDENCY_PRESENT';
  END IF;
  ...
  IF (SELECT count(*) FROM workflow_entity w
    ...
    RAISE EXCEPTION 'WF23_REMEDIATION_PROVIDER_NODE_CONTRACT_MISMATCH';
  END IF;
  ...
  IF (SELECT count(*) FROM workflow_entity w
    ...
    RAISE EXCEPTION 'WF23_REMEDIATION_CREDENTIAL_BINDING_MISMATCH';
  END IF;

  -- This is the exact observed orphan signature. It is intentionally not
  -- generalized to any terminal or live execution state.
  IF (SELECT count(*) FROM execution_entity e
      WHERE e.id = scope.execution_id
        AND e."workflowId" = scope.workflow_id
        AND e.status = 'running'
        AND e.finished = false
        AND e.mode = 'cli'
        AND e."waitTill" IS NULL
        AND e."stoppedAt" IS NULL
        AND e."deletedAt" IS NOT NULL
        AND e."storedAt" = 'db') <> 1
     OR (SELECT count(*) FROM execution_entity e WHERE e."workflowId" = scope.workflow_id) <> 1 THEN
    RAISE EXCEPTION 'WF23_REMEDIATION_ORPHAN_EXECUTION_CONTRACT_MISMATCH';
  END IF;
  ...
  IF (SELECT count(*) FROM workflow_history h JOIN workflow_entity w ON w.id = h."workflowId"
    ...
    RAISE EXCEPTION 'WF23_REMEDIATION_HISTORY_CONTRACT_MISMATCH';
  END IF;
  ...
  IF live_history IS DISTINCT FROM scope.expected_history THEN
    RAISE EXCEPTION 'WF23_REMEDIATION_CANONICAL_HISTORY_MISMATCH';
  END IF;
  ...
  LOOP
    ...
    expected_count := CASE reference.table_name
      ...
      WHEN 'execution_entity' THEN 1
      ELSE 0
    END;
    IF reference_count <> expected_count THEN
      RAISE EXCEPTION 'WF23_REMEDIATION_UNEXPECTED_WORKFLOW_REFERENCE';
    END IF;
  END LOOP;
  ...
  FOR reference IN
    ...
    JOIN pg_attribute attr ON attr.attrelid = con.conrelid AND attr.attnum = con.conkey[1]
    WHERE con.contype = 'f' AND array_length(con.conkey, 1) = 1
      AND con.confrelid = 'execution_entity'::regclass
  LOOP
    EXECUTE format('SELECT count(*) FROM %s WHERE %I = $1', reference.conrelid::regclass, reference.column_name)
      INTO reference_count USING scope.execution_id;
    expected_count := CASE reference.table_name WHEN 'execution_data' THEN 1 ELSE 0 END;
    IF reference_count <> expected_count THEN
      RAISE EXCEPTION 'WF23_REMEDIATION_UNEXPECTED_EXECUTION_REFERENCE';
    END IF;
  END LOOP;
END
$remediation_preflight$;

-- Transaction-local rollback backup: raw execution data never leaves
-- PostgreSQL and every copy is dropped at commit.
...
CREATE TEMP TABLE wf23_backup_execution ON COMMIT DROP AS
SELECT * FROM execution_entity WHERE id = (SELECT execution_id FROM remediation_scope);
CREATE TEMP TABLE wf23_backup_execution_data ON COMMIT DROP AS
SELECT d.* FROM execution_data d JOIN wf23_backup_execution e ON e.id = d."executionId";

DO $remediation_delete$
DECLARE
  target_workflow text := (SELECT workflow_id FROM remediation_scope);
  target_execution bigint := (SELECT execution_id FROM remediation_scope);
  affected integer;
BEGIN
  DELETE FROM execution_entity WHERE id = target_execution AND "workflowId" = target_workflow;
  GET DIAGNOSTICS affected = ROW_COUNT;
  IF affected <> 1 THEN RAISE EXCEPTION 'WF23_REMEDIATION_EXECUTION_DELETE_COUNT_MISMATCH'; END IF;

  DELETE FROM workflow_history WHERE "workflowId" = target_workflow;
  GET DIAGNOSTICS affected = ROW_COUNT;
  IF affected <> 1 THEN RAISE EXCEPTION 'WF23_REMEDIATION_HISTORY_DELETE_COUNT_MISMATCH'; END IF;

  DELETE FROM workflows_tags WHERE "workflowId" = target_workflow;
  GET DIAGNOSTICS affected = ROW_COUNT;
  IF affected <> 3 THEN RAISE EXCEPTION 'WF23_REMEDIATION_TAG_DELETE_COUNT_MISMATCH'; END IF;

  DELETE FROM shared_workflow WHERE "workflowId" = target_workflow;
  GET DIAGNOSTICS affected = ROW_COUNT;
  IF affected <> 1 THEN RAISE EXCEPTION 'WF23_REMEDIATION_SHARE_DELETE_COUNT_MISMATCH'; END IF;

  DELETE FROM workflow_entity WHERE id = target_workflow;
  GET DIAGNOSTICS affected = ROW_COUNT;
  IF affected <> 1 THEN RAISE EXCEPTION 'WF23_REMEDIATION_WORKFLOW_DELETE_COUNT_MISMATCH'; END IF;
  ...
  IF EXISTS (SELECT 1 FROM workflow_entity WHERE id = target_workflow)
     ...
     OR EXISTS (SELECT 1 FROM execution_entity WHERE id = target_execution)
     OR EXISTS (SELECT 1 FROM execution_data WHERE "executionId" = target_execution) THEN
    RAISE EXCEPTION 'WF23_REMEDIATION_DELETE_READBACK_MISMATCH';
  END IF;
END
$remediation_delete$;
