...
class AIEnrichmentEngine:
    ...
    def enrich(
        ...
        for policy in self.policies:
            ...
            unresolved = [
                ...
                if field == "tags"
                or (field not in locked and _unresolved(transaction, field))
            ]
            if not unresolved:
                continue
    ...
    def _apply_proposal(
        ...
        *,
        ...
        if isinstance(confidence_value, (int, float, str)) and not isinstance(
...
def record_ai_review(
    ...
    *,
    ...
    locked = set(transaction.metadata.get("locked_fields", []))
    current = transaction.tags if field == "tags" else transaction.value(field)
    desired = (
        set(str(tag) for tag in final_value)
        if field == "tags" and isinstance(final_value, list)
        else final_value
    )
    current_normalized = (
        {str(tag) for tag in current}
        if field == "tags" and isinstance(current, (set, list, tuple))
        else current
    )
    if field in locked:
        decision_status = "NOOP" if current_normalized == desired else "REJECTED"
        review = {
            "policy_id": policy_id,
            "field": field,
            ...
            "reviewer": reviewer,
            "reason": (
                "locked_field_already_has_value"
                if decision_status == "NOOP"
                else "locked_field_conflict"
            ),
        }
