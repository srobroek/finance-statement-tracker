...
import { ACTUAL_DATA_DIR, assertActualImportTransactions, assertActualWriterLease, assertIsoDate, assertObject, assertPreparedOutbox, requiredString } from './contracts';
import type { ActualCredential, ActualImportTransaction, JsonObject, PreparedActualOutbox } from './contracts';
...
export class ActualSession {
  ...
  async import(credentialValue: ActualCredential, input: unknown): Promise<JsonObject> {
    ...
    return this.run(credential, async api => {
      ...
      await api.sync();
      const readback = await readTransactionsByImportedIds(api, { account_id: outbox.account_id, imported_ids: outbox.transactions.map(row => row.imported_id), start_date: dates[0], end_date: dates.at(-1)! });
      const proof = verifyExactReadback(outbox.account_id, outbox.transactions, readback);
      const balanceAfter = await api.getAccountBalance(outbox.account_id);
      return {
        status: 'ACTUAL_OBSERVED', outbox_id: outbox.outbox_id, writer_lease: lease,
        imported_ids: outbox.transactions.map(row => row.imported_id), ...proof,
  ...
  async verify(credential: ActualCredential, input: unknown): Promise<JsonObject> {
    ...
    return this.run(credential, async api => {
      ...
      const proof = verifyExactReadback(request.account_id, request.expected_transactions, result);
      ...
      return {
        status: 'VERIFIED', ...result, mismatches: [], account_balance: accountBalance, ...proof,
      };
    });
  }
}
...
function canonicalObservedRow(row: ActualReturnedTransaction): EconomicRow {
  ...
  const cleared = row.cleared === undefined ? false : row.cleared;
  if (typeof cleared !== 'boolean') throw new Error('Actual returned transaction cleared must be boolean');
  return {
...
function verifyExactReadback(accountId: string, expectedRows: ActualImportTransaction[], result: { rows: ActualReturnedTransaction[]; missing_ids: string[]; duplicate_ids: string[] }): { expected_sha256: string; observed_sha256: string; transaction_count: number; amount_sum: number; imported_ids_verified: true } {
  if (result.missing_ids.length || result.duplicate_ids.length) throw new Error(`Actual readback failed; missing=${result.missing_ids.join(',')} duplicate=${result.duplicate_ids.join(',')}`);
  const expected = expectedRows.map(row => canonicalExpectedRow(accountId, row)).sort(byImportedId);
  const observed = result.rows.map(canonicalObservedRow).sort(byImportedId);
  const mismatches = expected.flatMap((row, index) => JSON.stringify(row) === JSON.stringify(observed[index]) ? [] : [row.imported_id]);
  if (expected.length !== observed.length || mismatches.length) throw new Error(`Actual readback field mismatch: ${mismatches.join(',')}`);
  return {
